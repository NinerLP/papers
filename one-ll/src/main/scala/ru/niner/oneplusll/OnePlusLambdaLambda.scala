...
class OnePlusLambdaLambda(val nodeNumber : Int, val edgeNumber : Int, val maximumCapacity : Int,
                          val lambda : Int, val mutationProbability : Double, val crossoverProbabilityForA : Double,
                          val fitnessFunction : FitnessFunction, val isAcyclic : Boolean, val computationsLimit : Int,
                          runID : Int) {
  val algorithmName = "1+" + lambda + "-" + lambda
  ...
  private def cross(): Unit = {
    ...
    for (i <- 0 until lambda) {
      crossoverGraphs.add(Graph.cross(bestMutantGraph, parentGraph, crossoverProbabilityForA))
  ...
  private def select(): Unit = {
    ...
    if (bestCrossoverGraph.fitnessValue > parentGraph.fitnessValue) log.println(computationsCount + " " + bestCrossoverGraph.fitnessValue)
