...
class DiffEvoPlus(val nodeNumber : Int, val maximumCapacity : Int,
      ...
      private def crossBestWithParents() : Unit = {
        val bestIntermediate = getBestIntermediate()

        for (i <- 0 until generationSize) {
          val newGraph = MatrixGraph.xorCross(parentGraphs.get(i), bestIntermediate)
