...
object SepCMAES {
  ...
  private class Impl[D: DomainType, C: CodomainType : CodomainComparator] (
    ...
    private object params {
      ...
      val muW = 1 / weights.view.map(i => i * i).sum

      val cS = (muW + 2) / (sizeD + muW + 3)
      val dS = 1 + cS + {
        val tmp = (muW - 1) / (sizeD + 1) - 1
        if (tmp >= 0) math.sqrt(tmp) else 0
      }
      val cC = 4 / (4.0 + sizeD)
      val d = 1 + 1 / cS

      val muCov = muW
      val cdCov1 = 2 / muCov / (sizeD + sqrt2) / (sizeD + sqrt2)
      val cdCov2 = (1 - 1 / muCov) * math.min(1, (2 * muCov - 1) / ((sizeD + 2) * (sizeD + 2) + muCov))
      val cCov = (sizeD + 2) / 3.0 * (cdCov1 + cdCov2)

      val aCov = 1.0 / muCov

      val pC_ZCoeff = math.sqrt(cC * (2 - cC) * muW)
      val pS_ZCoeff = math.sqrt(cS * (2 - cS) * muW)

      val ksiN = math.sqrt(sizeD) * (1 - 1.0 / (4 * sizeD) + 1.0 / (21 * sizeD * sizeD))
    }
    ...
    implicit val workingSetType = WorkingSetType[SepCMAES.WorkingSetType]
    implicit val initialization = Initialization().use {
      val rv = WorkingSet(
    ...
    implicit val iteration = Iteration() use { ws =>
      import params._
      import ws._
   ...
   */
  def equalWeights(offspringCount: Int, selectedCount: Int) = {
   ...
   * @return the weight sequence.
   */
  def logarithmicWeights(offspringCount: Int, selectedCount: Int) = {
