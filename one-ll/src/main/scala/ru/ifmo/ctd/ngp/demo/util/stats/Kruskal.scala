...
object Kruskal {
  ...
  private def lGamma1p(a: Double): Double = {
    if (math.abs(a) >= 0.5) Util.lgammafn(a + 1) else {
      ...
      for (i <- (0 until lGamma1pCoeffs.length).reverse) {
   ...
   * @return the p-value.
   */
  def rankSumTest[T: Numeric](data: Iterable[Iterable[T]]): Double = {
    if (data.size < 2) {
      Double.NaN
    } else {
      ...
      val kDen = 1 - (0 until ties.size).mapSum { t => val u = ties(t).toDouble; u * u * u - u} / (n.toDouble * n * n - n)
      if (kDen == 0) {
        Double.NaN
      } else {
        val k = kNum / kDen
