...
class MatrixOnePlusOnePaths(val nodeNumber : Int, val maximumCapacity : Int,
                            val fitnessFunction : NGPMatrixFitness,
                            val computationsLimit : Int, runID : Int) {
  val algorithmName = "1+1 Paths"
  ...
  private def mutate() : Unit = {
    ...
    val numberToMutate = 1// MathUtil.getBinomial(len, 1.0/len)
    //println(len + " " + numberToMutate)
    mutantGraph = MatrixGraph.pathMutate(parentGraph, numberToMutate)._1
