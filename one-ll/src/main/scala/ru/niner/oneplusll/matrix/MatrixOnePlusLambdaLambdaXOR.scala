...
class MatrixOnePlusLambdaLambdaXOR(val nodeNumber : Int, val maximumCapacity : Int,
                                   ...
                                   runID : Int) {
  val algorithmName = "1+" + lambda + "-" + lambda + "_probXOR"
  ...
  private def cross(): Unit = {
    ...
    for (i <- 0 until lambda) {
      crossoverGraphs.add(MatrixGraph.xorProbCross(bestMutantGraph, parentGraph, crossoverProbabilityForA))
