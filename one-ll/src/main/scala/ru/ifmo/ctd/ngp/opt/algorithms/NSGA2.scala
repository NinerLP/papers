...
import ru.ifmo.ctd.ngp.opt.{RandomSource, Evaluator, Evaluated, Iteration}
import ru.ifmo.ctd.ngp.opt.iteration.{Mutation, Update, Selection}
import ru.ifmo.ctd.ngp.opt.multicriteria.{DominationComparisonResult, DiversityMeasure, NonDominatedSorting, MultipleCriteria}
        ...
        def work(index: Int, collected: Int) {
          if (collected < size) {
            ...
            if (collected + currSize <= size) {
              ...
              val order = (0 until crowd.size).sortBy(crowd)
