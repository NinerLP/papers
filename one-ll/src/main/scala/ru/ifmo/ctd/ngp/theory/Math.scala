...
object Math {
  ...
  implicit object Fraction extends Fractional[Fraction] {
    ...
    def toDouble(x: Fraction) = (x.numerator * 1000000000000000000L / x.denominator).doubleValue() / 1e18
    def compare(x: Fraction, y: Fraction) = x.numerator * y.denominator compare y.numerator * x.denominator
  }
  implicit def int2fraction(that: Int) = new Fraction(that, 1)
  implicit def long2fraction(that: Long) = new Fraction(that, 1)
  implicit def bigInt2fraction(that: BigInt) = new Fraction(that, 1)
  ...
  def solveTridiagonal[T: Fractional](a: IndexedSeq[T], b: IndexedSeq[T], c: IndexedSeq[T], x: IndexedSeq[T]) = {
    ...
    cc(0) = num.div(cc(0), b(0))
    xx(0) = num.div(xx(0), b(0))
